# -*- coding: utf-8 -*-
"""
Created on Sun Jun 28 19:58:44 2026

@author: mahad
"""

# file: 06_Run_TwoStep_GJR_GARCH_Models.py
...
def get_first_value(x):
    """Safely gets the first value from either a pandas Series or numpy array."""
    if hasattr(x, "iloc"):
        return x.iloc[0]
    return x[0]
...
def clark_west_test(actual, benchmark_forecast, extended_forecast):
    ...
    t_stat = get_first_value(model.tvalues)
...
def estimate_raw_gjr_garch_forecast(training_returns, horizon_days):
    """
    Estimate GJR-GARCH(1,1) on daily returns and forecast target-month volatility.
    ...
    - daily variance forecasts are in percent-squared units
    ...
    try:
        ...
        fitted = gjr_garch.fit(disp="off", show_warning=False)
...
# ==================================================
def main():
    print("Running two-step GJR-GARCH forecast-augmentation models...")
    ...
    raw_gjr_output_file = base_path / "GJR_GARCH_TwoStep_Raw_GJR_Forecasts.xlsx"
    forecasts_output_file = base_path / "GJR_GARCH_TwoStep_Model_Forecasts.xlsx"
    performance_output_file = base_path / "GJR_GARCH_TwoStep_Model_Performance.xlsx"
    coefficients_output_file = base_path / "GJR_GARCH_TwoStep_Model_Coefficients.xlsx"
    summary_output_file = base_path / "GJR_GARCH_TwoStep_Model_Results.txt"
    ...
    # --------------------------------------------------
    # STEP 1: CREATE RAW GJR-GARCH FORECASTS
    # --------------------------------------------------
    print("Step 1: Estimating raw GJR-GARCH forecasts...")
    ...
    for i, row in df_monthly.iterrows():
        ...
        print(f"Raw GJR-GARCH {i + 1}/{len(df_monthly)}: target month {target_month.date()}")
        ...
        raw_row = {
            ...
            "Raw GJR-GARCH Forecast Annualized Volatility (%)": np.nan,
            "Raw GJR-GARCH Forecast Log Volatility": np.nan,
            "Raw GJR-GARCH Forecast Error": np.nan,
            "Daily Training Observations": np.nan,
            "GJR-GARCH mu": np.nan,
            "GJR-GARCH omega": np.nan,
            "GJR-GARCH alpha": np.nan,
            "GJR-GARCH gamma": np.nan,
            "GJR-GARCH beta": np.nan,
            "GJR-GARCH persistence": np.nan,
            "GJR-GARCH log likelihood": np.nan,
            "GJR-GARCH AIC": np.nan,
            "GJR-GARCH BIC": np.nan,
            "GJR-GARCH convergence flag": np.nan,
            "Raw GJR-GARCH Error Message": "",
        }

        origin_daily = df_daily[df_daily["Month"] == origin_month].copy()

        if len(origin_daily) == 0:
            raw_row["Raw GJR-GARCH Error Message"] = "No daily returns found for origin month"
            raw_gjr_rows.append(raw_row)
            continue
        ...
        gjr_result = estimate_raw_gjr_garch_forecast(train_returns, horizon_days)

        if gjr_result["success"]:
            raw_row["Raw GJR-GARCH Forecast Annualized Volatility (%)"] = gjr_result["forecast_annualized_volatility"]
            raw_row["Raw GJR-GARCH Forecast Log Volatility"] = gjr_result["forecast_log_volatility"]
            raw_row["Raw GJR-GARCH Forecast Error"] = (
                row["Next Month Log Realized Volatility"] - gjr_result["forecast_log_volatility"]
            )
            raw_row["GJR-GARCH mu"] = gjr_result["mu"]
            raw_row["GJR-GARCH omega"] = gjr_result["omega"]
            raw_row["GJR-GARCH alpha"] = gjr_result["alpha"]
            raw_row["GJR-GARCH gamma"] = gjr_result["gamma"]
            raw_row["GJR-GARCH beta"] = gjr_result["beta"]
            raw_row["GJR-GARCH persistence"] = gjr_result["persistence"]
            raw_row["GJR-GARCH log likelihood"] = gjr_result["log_likelihood"]
            raw_row["GJR-GARCH AIC"] = gjr_result["aic"]
            raw_row["GJR-GARCH BIC"] = gjr_result["bic"]
            raw_row["GJR-GARCH convergence flag"] = gjr_result["convergence_flag"]
        else:
            raw_row["Raw GJR-GARCH Error Message"] = gjr_result["error_message"]
            raw_row["GJR-GARCH convergence flag"] = gjr_result["convergence_flag"]

        raw_gjr_rows.append(raw_row)

    raw_gjr_df = pd.DataFrame(raw_gjr_rows)

    # Merge raw GJR-GARCH forecasts into monthly dataset
    model_df = df_monthly.merge(
        raw_gjr_df[
            [
                "Month",
                "Target Month",
                "Raw GJR-GARCH Forecast Annualized Volatility (%)",
                "Raw GJR-GARCH Forecast Log Volatility",
    ...
    raw_gjr_col = "Raw GJR-GARCH Forecast Log Volatility"
    agg_col = "Aggregate Climate Risk"
    pca_col = "PCA Climate Risk Index"

    oos_df = model_df[model_df["Sample Split"].str.lower() == "out-of-sample"].copy()
    ...
    for i, row in oos_df.iterrows():
        ...
        forecast_row = {
            ...
            "Raw GJR-GARCH Forecast Annualized Volatility (%)": row["Raw GJR-GARCH Forecast Annualized Volatility (%)"],
            "Raw GJR-GARCH Forecast Log Volatility": row[raw_gjr_col],
            "Two-Step Benchmark GJR-GARCH Forecast Log Volatility": np.nan,
            "Two-Step Benchmark GJR-GARCH Forecast Annualized Volatility (%)": np.nan,
            "Two-Step Benchmark GJR-GARCH Forecast Error": np.nan,
            "Two-Step GJR-GARCH + Aggregate CRI Forecast Log Volatility": np.nan,
            "Two-Step GJR-GARCH + Aggregate CRI Forecast Annualized Volatility (%)": np.nan,
            "Two-Step GJR-GARCH + Aggregate CRI Forecast Error": np.nan,
            "Two-Step GJR-GARCH + PCA CRI Forecast Log Volatility": np.nan,
            "Two-Step GJR-GARCH + PCA CRI Forecast Annualized Volatility (%)": np.nan,
            "Two-Step GJR-GARCH + PCA CRI Forecast Error": np.nan,
        ...
        # ------------------------------
        # MODEL 1: BENCHMARK TWO-STEP GJR-GARCH
        # ------------------------------
        benchmark_result = fit_ols_and_predict(
            train_df=train_base,
            y_col=y_col,
            predictor_cols=[raw_gjr_col],
            current_values={raw_gjr_col: row[raw_gjr_col]},
            min_train_obs=30,
        )
        ...
        if benchmark_result["success"]:
            ...
            forecast_row["Two-Step Benchmark GJR-GARCH Forecast Log Volatility"] = benchmark_forecast
            forecast_row["Two-Step Benchmark GJR-GARCH Forecast Annualized Volatility (%)"] = np.exp(benchmark_forecast)
            forecast_row["Two-Step Benchmark GJR-GARCH Forecast Error"] = actual_value - benchmark_forecast

            coeff_row = {
                "Target Month": current_target_month,
                "Model": "Two-Step Benchmark GJR-GARCH",
        ...
        # ------------------------------
        # MODEL 2: GJR-GARCH + AGGREGATE CRI
        # ------------------------------
        train_agg = train_base.dropna(subset=[y_col, raw_gjr_col, agg_col]).copy()
        ...
        if len(train_agg) >= 30:
            ...
            if pd.notna(agg_std) and agg_std != 0:
                ...
                agg_result = fit_ols_and_predict(
                    train_df=train_agg,
                    y_col=y_col,
                    predictor_cols=[raw_gjr_col, "Aggregate CRI Std"],
                    current_values={
                        raw_gjr_col: row[raw_gjr_col],
                ...
                if agg_result["success"]:
                    ...
                    forecast_row["Two-Step GJR-GARCH + Aggregate CRI Forecast Log Volatility"] = agg_forecast
                    forecast_row["Two-Step GJR-GARCH + Aggregate CRI Forecast Annualized Volatility (%)"] = np.exp(agg_forecast)
                    forecast_row["Two-Step GJR-GARCH + Aggregate CRI Forecast Error"] = actual_value - agg_forecast

                    coeff_row = {
                        "Target Month": current_target_month,
                        "Model": "Two-Step GJR-GARCH + Aggregate CRI",
        ...
        # ------------------------------
        # MODEL 3: GJR-GARCH + PCA CRI
        # ------------------------------
        train_pca = train_base.dropna(subset=[y_col, raw_gjr_col, pca_col]).copy()
        ...
        if len(train_pca) >= 30:
            ...
            if pd.notna(pca_std) and pca_std != 0:
                ...
                pca_result = fit_ols_and_predict(
                    train_df=train_pca,
                    y_col=y_col,
                    predictor_cols=[raw_gjr_col, "PCA CRI Std"],
                    current_values={
                        raw_gjr_col: row[raw_gjr_col],
                ...
                if pca_result["success"]:
                    ...
                    forecast_row["Two-Step GJR-GARCH + PCA CRI Forecast Log Volatility"] = pca_forecast
                    forecast_row["Two-Step GJR-GARCH + PCA CRI Forecast Annualized Volatility (%)"] = np.exp(pca_forecast)
                    forecast_row["Two-Step GJR-GARCH + PCA CRI Forecast Error"] = actual_value - pca_forecast

                    coeff_row = {
                        "Target Month": current_target_month,
                        "Model": "Two-Step GJR-GARCH + PCA CRI",
    ...
    benchmark_forecast_col = "Two-Step Benchmark GJR-GARCH Forecast Log Volatility"
    agg_forecast_col = "Two-Step GJR-GARCH + Aggregate CRI Forecast Log Volatility"
    pca_forecast_col = "Two-Step GJR-GARCH + PCA CRI Forecast Log Volatility"
    ...
    performance_rows.append(
        {
            "Model name": "Two-Step Benchmark GJR-GARCH",
    ...
    performance_rows.append(
        {
            "Model name": "Two-Step GJR-GARCH + Aggregate CRI",
    ...
    performance_rows.append(
        {
            "Model name": "Two-Step GJR-GARCH + PCA CRI",
    ...
    raw_gjr_df.to_excel(raw_gjr_output_file, index=False)
    ...
    if len(performance_df) > 0 and performance_df["MSE / MSFE"].notna().any():
        lowest_msfe_model = performance_df.loc[
            performance_df["MSE / MSFE"].idxmin(),
            "Model name",
        ]
        lowest_rmse_model = performance_df.loc[
            performance_df["RMSE"].idxmin(),
            "Model name",
        ]
    else:
    ...
    summary_text = f"""
TWO-STEP GJR-GARCH FORECAST-AUGMENTATION MODEL RESULTS
======================================================
...
Step 1:
Estimate GJR-GARCH(1,1) using daily S&P 500 returns.

GJR-GARCH variance equation:
h_t = omega + alpha * epsilon_(t-1)^2 + gamma * I(epsilon_(t-1) < 0) * epsilon_(t-1)^2 + beta * h_(t-1)

Where:
- h_t is conditional variance.
- alpha captures the effect of the previous squared shock.
- gamma captures the additional effect of negative shocks.
- beta captures volatility persistence.
- I(epsilon_(t-1) < 0) equals 1 when the previous shock is negative and 0 otherwise.

Raw GJR-GARCH forecast:
Daily GJR-GARCH variances are forecast for each trading day of the target month.
...
Model 1: Two-Step Benchmark GJR-GARCH
Next Month Log Realized Volatility = alpha + beta × Raw GJR-GARCH Forecast Log Volatility

Model 2: Two-Step GJR-GARCH + Aggregate CRI
Next Month Log Realized Volatility = alpha + beta × Raw GJR-GARCH Forecast Log Volatility + gamma × Aggregate CRI

Model 3: Two-Step GJR-GARCH + PCA CRI
Next Month Log Realized Volatility = alpha + beta × Raw GJR-GARCH Forecast Log Volatility + gamma × PCA CRI

Important methodology note:
- This is the two-step GJR-GARCH forecast-augmentation model.
- Climate risk does not enter the daily GJR-GARCH variance equation.
- Instead, climate risk is used as an additional monthly predictor after the raw GJR-GARCH forecast has been created.
- This model tests whether climate risk adds incremental predictive information beyond asymmetric GJR-GARCH volatility forecasts.

Daily return unit:
- Daily Return (%) is used.
- Therefore GJR-GARCH variance forecasts are in percent-squared units.
...
- Two-Step Benchmark GJR-GARCH: {forecasts_df[benchmark_forecast_col].notna().sum() if len(forecasts_df) > 0 else 0}
- Two-Step GJR-GARCH + Aggregate CRI: {forecasts_df[agg_forecast_col].notna().sum() if len(forecasts_df) > 0 else 0}
- Two-Step GJR-GARCH + PCA CRI: {forecasts_df[pca_forecast_col].notna().sum() if len(forecasts_df) > 0 else 0}
...
- Positive R2_OS means the climate-augmented model improves over the two-step benchmark GJR-GARCH model.
- Negative R2_OS means the climate-augmented model performs worse than the two-step benchmark GJR-GARCH model.
...
"""
    ...
    print(summary_text)
    print("Two-step GJR-GARCH model completed successfully.")
